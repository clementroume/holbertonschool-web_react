...
test("Should display 3 notification items as expected", () => {
  ...
  const reactPropsKey = Object.keys(notificationsListItems[2]).find((key) =>
    /^__reactProps/.test(key)
  );
  if (reactPropsKey) {
    const dangerouslySetInnerHTML =
      notificationsListItems[2][reactPropsKey].dangerouslySetInnerHTML.__html;
    expect(dangerouslySetInnerHTML).toContain(
...
test("Should display the correct notification colors", () => {
  ...
  for (let i = 0; i <= notificationsListItems.length - 1; i++) {
    const styleProp = Object.keys(notificationsListItems[i]).find((key) =>
      /^__reactProps/.test(key)
    );
    if (styleProp) {
...
test.skip("Should rerender when prop values change", () => {
  ...
  const updatedProps = {
    ...initialProps,
    notifications: [{ id: 2, type: "urgent", value: "Urgent notification" }],
  };
